...
# =============================================================================
# Scenario Models (Multi-Scenario Orchestration)
# =============================================================================
...
class ScenarioDefinition(BaseModel):
    ...
    This is an internal model for LLM output. Each scenario represents
    a different capital ask with potentially different constraints.

    v1: Minimal control surface. Only target and two key constraints.
    """

    label: str = Field(
        ..., min_length=1, max_length=50, description="Short label (e.g., 'Conservative')"
    )
    kind: ScenarioKind = Field(..., description="Scenario classification")
    rationale: str = Field(
        ..., min_length=1, max_length=200, description="1-2 sentence rationale for this variant"
    )

    # v1: Only three numeric knobs
    target_amount: float = Field(..., gt=0, description="Target proceeds for this scenario")
    max_leverage: Optional[float] = Field(
        None, gt=0, lt=10, description="Max net leverage (None = use default)"
    )
    min_coverage: Optional[float] = Field(
        None, gt=0, lt=20, description="Min fixed charge coverage (None = use default)"
    )
